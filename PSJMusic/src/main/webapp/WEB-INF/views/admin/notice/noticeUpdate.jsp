...
<title>공지사항 작성</title>
    ...
    function fCheck() {
    	...
    	var SJ_NOTICE_CONTENT = myform.SJ_NOTICE_CONTENT.value;
			...
			<div class="container">
				<form name="myform" method="post">
					...
					<table class="table">
						...
						<tr>
							<th>글제목</th>
							<td><input type="text" name="SJ_NOTICE_TITLE" value="${vo.SJ_NOTICE_TITLE}" placeholder="글제목을 입력하세요" class="form-control" autofocus required /></td>
						</tr>
						<tr>
							<th>글내용</th>
							<td><textarea rows="6" name="SJ_NOTICE_CONTENT" id="CKEDITOR" class="form-control" required >${vo.SJ_NOTICE_CONTENT}</textarea></td>
						</tr>
						<tr>
							<th>공지옵션</th>
							<td>
								<input type="radio" name="SJ_NOTICE_NOTICE" value="1" <c:if test="${fn:contains(vo.SJ_NOTICE_NOTICE,'1')}">checked</c:if>>Main공지사항
         			  <input type="radio" name="SJ_NOTICE_NOTICE" value="0" <c:if test="${fn:contains(vo.SJ_NOTICE_NOTICE,'')}">checked</c:if>>비공지
	         		</td>
						</tr>
						<tr>
					...
					<input type="hidden" name="SJ_NOTICE_IDX" value="${vo.SJ_NOTICE_IDX}"/>
					<input type="hidden" name="pag" value="${pag}"/>
					<input type="hidden" name="pagSize" value="${pagSize}"/>
					<input type="hidden" name="oriContent" />
					<div id="oriContent" style="display:none;">${vo.SJ_NOTICE_CONTENT}</div>
				</form>
			</div>
   	</div>
<script>
